...
function mostra_trocar_senha() {
  $("#tabela_trocar_senha").show();
}

function cancelar_trocar_senha() {
  $("#tabela_trocar_senha").hide();
}

function confirma_trocar_senha(id) {
  senha1 = document.getElementById("senha1").value;
  senha2 = document.getElementById("senha2").value;
  if (senha1.length < 6) {
    alert('A senha deve possuir pelo menos 6 caracteres ou números.');
    return;
  }
  if (!(senha1 == senha2)) {
    alert('As senhas não conferem. Digite a mesma senha nos dois campos.');
    return;
  }
  xmlhttp = new ajax();
  var destino = home + 'controller/ControllerUsuarioArea.php?acao=trocar_senha&id=' + id + '&senha=' + senha1;
  javascript:window.history.forward(1);
  noBack();
  xmlhttp.enviar(destino, "POST", false);
  $("#tabela_trocar_senha").hide();
...
function mudarStatus(idrole, role, status, str) {
    if (confirm('Tem certeza que deseja alterar o status do ' + role + ' ? ')) {
        xmlhttp = new ajax();
        var destino = home + 'controller/ControllerMudarStatus.php?id' + role + '=' + idrole
                + "&status=" + status + "&role=" + role;
        xmlhttp.enviar(destino, "POST", false);
        $('#status_'+role+'_'+idrole).html(str);
    }
}