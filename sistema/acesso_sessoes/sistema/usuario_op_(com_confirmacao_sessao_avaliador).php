...
# Retorno: areas do AVALIADOR -> home_areaAvaliador.php #	
	...
	$sql = "SELECT * FROM area a LEFT JOIN avaliador_area aa ON a.id_area = aa.fk_area AND fk_avaliador =".$id_user;
	$query = runSQL($sql);
	...
	CASE s.modalidade
		...
		ELSE ''
		END as modalidade
	FROM sessao s, avaliador_sessao avs WHERE avs.fk_avaliador =".$id_user." AND avs.status <> 2 AND s.id_sessao = avs.fk_sessao ORDER BY s.data ASC, s.hora_ini ASC";
	...
	while($linha = mysql_fetch_array($result)) {
		...
		if($linha["status_aval"] == 0) 
			$str .= '<fieldset style="background-color:#FFE4E1;"> Atividade Pendente: ';
		else if($linha["status_aval"] == 1) 
			$str .= '<fieldset style="background-color:#CCDAB4;"> Atividade Confirmada: ';
...
# Retorno: dados do VOLUNTARIO -> home_areaVoluntario.php #	
	...
	$id_user = (int)$_SESSION["id_usuario"];
	$sql = "SELECT v.fk_curso, c.nome as nomeCurso, c.nivel, c.fk_campus, ca.nome as nomeCampus, ca.fk_instituicao, i.nome as nomeInst, i.sigla,
			CASE c.nivel
				WHEN 2 THEN '(Técnico) - '
				WHEN 3 THEN '(Superior) - '
				ELSE '' 
...
} else if(isset($_POST["opcao"]) && $_POST["opcao"]=="sair") {
	session_destroy();
    echo 1;
}
