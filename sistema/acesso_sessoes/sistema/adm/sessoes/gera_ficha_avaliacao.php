...
$array_tematica[8]  = "Lingüística, Letras e Artes";
	...
	$sql_orientadores= "SELECT t.id_trabalho, toc.seq, u.nome FROM usuario u
INNER JOIN orientador o ON o.fk_usuario = u.id_usuario
INNER JOIN orientador_campus oc ON oc.fk_orientador = o.fk_usuario 
INNER JOIN trabalho_orientador_campus toc ON toc.fk_campus = oc.fk_campus AND toc.fk_orientador = oc.fk_orientador
INNER JOIN trabalho t ON t.id_trabalho = toc.fk_trabalho 
WHERE t.id_trabalho = ".$id_trabalho." AND t.status = 4
order by toc.seq";

   $result_orientadores = runSQL($sql_orientadores);
   if($result_orientadores == false)
	echo mysql_error();
	$orientadores = "";
...
$var_html = "";
$var_html=$var_html."<div style='width:170mm;margin-left:10mm; page-break-after: always; @charset: UTF-8;'>";
  
$cabecalho_tabela = "<table width='100%' border='0' cellpadding='3' cellspacing='0' align='center' style='margin-left:1cm;font-family:arial;font-size:11pt;'>";
$var_html=$var_html.$cabecalho_tabela;
$var_html=$var_html."<tr>
<td width='150' align='left' valign='middle' bgcolor='#ffffff'>
<img src='images/Logo_IF_Campus_POA_2cm.png' width='150' border='0' align='left'><br><br></td>

<td align='center' valign=middle bgcolor='#ffffff' style='font-weight:bold;font-size:13pt;'>
15ª Mostra de Pesquisa, Ensino e Extensão<br>
IFRS - Câmpus Porto Alegre</td>
</tr>

<tr><td colspan=2 align=center style='font-weight:bold;'>
<br>FICHA DE AVALIAÇÃO DE TRABALHO<br><br>
<b>Modalidade:</b> ".$array_modalidade[$modalidade]."<br>".
"</td>
</tr>";

$var_html=$var_html."<tr><td colspan=2 align=left><b>Nº do trabalho:</b> ".str_pad($id_trabalho, 3, "0", STR_PAD_LEFT)."<br>".
"<b>Título do trabalho:</b> ".$titulo."<br>".
"<b>Nome do apresentador:</b> ".$nome_apresentador."<br>".
"<b>Orientador(es):</b> ".$orientadores."<br>".
...
"O Orientador ou Co-orientador estava presente: (&nbsp;&nbsp;&nbsp;)Sim  (&nbsp;&nbsp;&nbsp;)Não <BR>".
