...
<?php 
    $sql_colunas = "SELECT t.id_trabalho, t.fk_area, t.fk_modalidade, upper(t.titulo_ordenar) as titulo, t.status, s.id_sessao, s.numero, s.nome AS nome_sessao, s.nome_sala, s.data, s.hora_ini, s.hora_fim, a.nome AS nome_area 
    FROM trabalho t 
    LEFT JOIN sessao s ON s.id_sessao = t.fk_sessao 
    LEFT JOIN area a ON t.fk_area = a.id_area 
    where t.status=4 ";
    ...
    else if ($ordenar == 'area')
      $sql_trabalho= $sql_colunas." order by a.id_area, t.fk_modalidade, s.numero;";
    else if ($ordenar == 'modalidade')
      $sql_trabalho= $sql_colunas." order by t.fk_modalidade, t.fk_area, s.numero;";
    else if ($ordenar == 'sessao') 
      $sql_trabalho= $sql_colunas." order by s.nome, t.fk_modalidade, t.fk_area, t.titulo_ordenar;";
  ...
  $num_cols++; //pois tem o titulo do trabalho.
  if (isset($col_status)){
  ...
  $tabela.= "<table border=1 cellpadding=0 cellspacing=0 width=100%>";
  if ($ordenar == 'id') {
  ...
  while ($i > 0) {
    if ($ordenar =='area') {
      if ($area != $linha_trabalho['fk_area']) {
        $area = $linha_trabalho['fk_area'];
