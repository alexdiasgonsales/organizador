...
$sql_trab= "SELECT t.id_trabalho, t.titulo_ordenar, t.fk_tematica, t.fk_categoria, t.fk_modalidade, c.nivel, u.nome AS nome_autor
FROM trabalho t, trabalho_autor_curso tac, usuario u, curso c
WHERE (t.id_trabalho = ".$id_trabalho." AND t.status = 2) AND (tac.fk_trabalho = t.id_trabalho AND tac.seq = 1) AND u.id_usuario = tac.fk_autor AND (c.id_curso = tac.fk_curso)
...
while ($linha_avaliador = mysql_fetch_array($result_avaliador)) { 
	...
	$tematica= $linha_trab['fk_tematica'];
...
$var_html="<html><head><meta http-equiv='Content-Type' content='text/html; charset=utf-8'><style> p {  text-align: justify; } td {border-style:solid;border-color:white;border-width:1px;padding:1px:margin:1px;} </style></head><body style='width:210mm;margin-left:0mm;padding:0px;'>";
...
<td align='center' valign=top bgcolor='#ffffff' style='font-weight:bold;font-size:16pt;'>14ª Mostra de Pesquisa, Ensino e Extensão<br>
