...
##############################################################################################################################
  ...
  while ($row = mysql_fetch_array($queryAvaliadores)) {
    if ($row["statusAvaliador"] == "A")
      $str .= '<tr><td><input type="checkbox" name="aceita[]" value="' . $row["fk_usuario"] . '" checked="true"></td>';
    else
      $str .= '<tr><td><input type="checkbox" name="aceita[]" value="' . $row["fk_usuario"] . '"></td>';
    ...
    $str .= "<td>" . $row["tipoServidor"] . "</td>";
    $str .= "<td>" . $row["statusAvaliador"] . "</td>";
    $str .= '<td><a href="#" class="link1" onclick="aceitaAvaliador(' . $row["fk_usuario"] . ');" style="margin-left:20px;font-size:10px;">Ac.</a></td>';
    $str .= "<td><a href=\"#\" class=\"link1\" onclick=\"recusaAvaliador(" . $row["fk_usuario"] . ");\" style=\"margin-left:20px;font-size:10px;\">Rec.</a></td>";
...
##############################################################################################################################
  ...
  $sql_recupera_areas_av = "SELECT a.id_area, a.nome, aa.fk_area FROM avaliador_area aa, tematica a WHERE aa.fk_avaliador =" . $id_av . " AND a.id_area = aa.fk_area";
  ...
  $str .= '<a href="#" style="link1" onclick="voltar();">voltar</a>';
...
##############################################################################################################################
  ...
  while ($row = mysql_fetch_array($queryVoluntarios)) {
    ...
    $str .= "<tr><td style=\"border-bottom:1px solid #CCDAB4;\">" . $row["id_usuario"] .
            ...
            "</td><td style=\"border-bottom:1px solid #CCDAB4;\">" . $row["Telefone2"] .
            "</td><td style=\"border-bottom:1px solid #CCDAB4;\">" . $row["Telefone3"] .
            ...
            "</td><td><a href=\"#\" class=\"link1\" title=\"Clique para " . $optionV . " voluntário\" onclick=\"alteraStatusVoluntario(" . $row["id_usuario"] . ",'$optionV');\" style=\"margin-left:20px;font-size:10px;\">$textoV</a>" .
            "</td><td><a href=\"#\" class=\"link1\" title=\"Clique para " . $preseV . " presença\" onclick=\"alteraPresencaVoluntario(" . $row["id_usuario"] . ",'$preseV');\" style=\"margin-left:20px;font-size:10px;\">$textoPreseV</a>" .
            "</td></tr>";
  }
...
##############################################################################################################################
  ...
  if (isset($_GET["ordem"]))
    ...
    $ordem = 1;
  //$sqlTrabalhos = "SELECT t.id_trabalho, t.titulo, t.status, a.nome FROM trabalho t inner join tematica a on a.id_area = t.fk_tematica ORDER BY a.nome ASC";
  ...
  if ($ordem == 1)
    ...
    $sqlTrabalhos .= "ORDER BY t.fk_categoria, t.fk_modalidade, t.fk_tematica, t.id_trabalho ";
  else if ($ordem == 8)
  ...
  while ($row = mysql_fetch_array($queryTrabalhos)) {
    $str .= "<tr valign=top><td>" . $row["id_trabalho"] .
            ...
            "</td><td>" . $row["sigla"].
            //"</tr>";
            "</td><td><a href=\"analisaTrab.php?id_trab=" . $row["id_trabalho"] . "\" class=\"link1\">Ver</a></td></tr>";
  }
...
##############################################################################################################################
  ...
  //Contabiliza total trabalhos por área:
  $str .= "<tr><td colspan=10 style='background-color:#CCDAB4;'> TOTALIZAÇÃO TRABALHOS POR ÁREA </td></tr>";
  $sqlAreas = "SELECT t.fk_tematica, a.nome, count(*) AS quant FROM trabalho t
    INNER JOIN tematica a ON a.id_area = t.fk_tematica
    GROUP BY t.fk_tematica ORDER BY t.fk_tematica, a.nome";
  $queryAreas = runSQL($sqlAreas);
  $str .= "<tr><td colspan=10>";
  while ($rowArea = mysql_fetch_array($queryAreas)) {
    $str .= "(" . $rowArea["fk_tematica"] . ") " . $rowArea["nome"] . " = " . $rowArea["quant"] . "<br>";
  }
  $str .= "</td></tr>";
...
##############################################################################################################################
  ...
  if (isset($_SESSION["id_administracao"])) {
    ...
    if (true) {
      ...
      $str .= '<div style="padding-left:10px;height:30px;padding-top:5px;"> ' . $arr_status_trab[$status] . ': '. $arr_status_trab_completo[$status] . ' </div>';

      //$str .= '<br><a href="../imprimir_trabalho.php?id_trabalho=' . $id_trab . '" class="link1" style="margin-left:20px;font-size:10px;text-decoration:underline;height:30px;padding-bottom:15px;">Visualizar PDF</a>';
      //$str .= '<a href="adm_trabalhos.php" class="link1" style="margin-left:20px;font-size:10px;text-decoration:underline;height:30px;padding-bottom:15px;">Voltar</a>';
    } else {
      $str = "Error:";
    }
  } else {
...
##############################################################################################################################
  ...
  if (isset($id_trab)) {
    ...
    //if ($statusTrabalho != STATUS_TRAB_PENDENTE && ($contaLinhas == 0 || ($contaLinhas == 1 && ETAPA_ANALISE_TRABALHO_2 == 1))) {
    if ( ($contaLinhas == 0 || ($contaLinhas == 1 && ETAPA_ANALISE_TRABALHO_2 == 1) ) ) {
    ...
    else if ($contaLinhas == 2 || ($contaLinhas == 1 && (ETAPA_INSCRICAO_TRABALHO == 1 || ETAPA_ANALISE_TRABALHO_1 == 1))) {
...
##############################################################################################################################
  ...
  $string = '<label style="font-weight: bold;">Parecer </label> ' . $seq . '<br><br>';
  ...
  $string .= '<div style="clear:both; height:10px;"></div>';
  $string .= '<label style="font-weight: bold;">Observações Gerais:</label> ' . stripslashes($linha_analise["observacoes"]) . '<br><br>';
  if (isset($_SESSION["id_administracao"])) {
    $string .= '<label style="font-weight: bold;">Observações Internas:</label> ' . stripslashes($linha_analise["observacoes_internas"]) . '<br><br>';
  }
...
##############################################################################################################################
  ...
  if ($status == STATUS_TRAB_ACEITO) {
    ...
    $assunto = "Mostra IFRS POA 2015 - trabalho aceito";
  } else if ($status == STATUS_TRAB_CORRIGIR) {
    $msg = "foi analisado pela Comissão Organizadora do evento e necessita de correções.\n
      O autor principal deverá entrar no sistema para visualizar o parecer e efetuar as correções, seguindo os seguintes passos:\n\n
      1) Acessar o Sistema de Inscrição e clicar em Área do Autor.\n
      2) Localizar o trabalho e clicar no link Ver/Modificar...\n
      3) Visualizar o parecer emitido pela Comissão Organizadora do evento.\n
	  4) Efetuar as devidas correções, conforme solicitado.\n
      Observação: atentar para fato de verificar se o trabalho possui pelo menos um orientador vinculado (conforme regulamento do evento), e que para vincular o orientador, este deve estar cadastrado no sistema.\n\n
      5) Clicar em Salvar Trabalho.\n
	  Após isso, deverá ser confirmado o envio do trabalho:\n
      1) Localizar o trabalho e clicar no link Ver/Validar...\n
      2) Clicar no botão Enviar Trabalho.\n
      A data limite foi prorrogada de 22/08/2014 para 25/08/2014.\n\n";
    $assunto = "Mostra IFRS POA 2014 - trabalho necessita correções (data limite 25/08/2014).";
    ...
    $assunto = "Mostra IFRS POA 2014 - trabalho recusado";
  } 
  else if ($status == STATUS_TRAB_PENDENTE) {
    $assunto = "Mostra IFRS POA 2015 - trabalho PENDENTE (prazo estendido para 21/08/2015).";
    $msg = "encontra-se em estado \"PENDENTE\".

      O autor principal deve confirmar o envio do trabalho seguindo os seguintes passos:
      
      1) Efetuar login no sistema e acessar a Área do Autor.
      2) Localizar o trabalho e clicar no link Ver/Modificar/Enviar...
      3) Clicar no botão Enviar.
      Observação: atentar para fato de verificar se o trabalho possui pelo menos um orientador vinculado (conforme regulamento do evento) e que, para vincular o orientador, este deve estar cadastrado no sistema.
      
      Após isso, o orientador deverá VALIDAR o envio do trabalho (conforme previsto no regulamento), seguindo os seguintes passos:
      1) Efetuar login no sistema e acessar a Área do Orientador.
      2) Localizar o trabalho e clicar no link Ver/Validar...
      3) Clicar no botão Validar.

      A data limite foi estendida para 21/08/2015 - 23:59h.\n\n";
  }
  else if ($status == STATUS_TRAB_ENVIADO) {
    $assunto = "Mostra IFRS POA 2015 - trabalho necessita VALIDAÇÃO do orientador (prazo estendido para 21/08/2015).";
    $msg = "foi enviado pelo autor principal mas precisa ser confirmado pelo orientador (conforme previsto no regulamento).\n
      Para isso, o orientador deve seguir os seguintes passos:\n
      1) Efetuar login no sistema e acessar a Área do Orientador.
      2) Localizar o trabalho e clicar no link Ver/Validar...
      3) Clicar no botão Validar.
  ...
  while ($row = mysql_fetch_array($executa_selecao_trab)) {
    ...
    $titulo = $row["titulo"];
    $titulo = strip_tags(html_entity_decode($titulo));
    
    $mensagem = "Prezado(s) autor(es) e orientador(es),\n\n";
    $mensagem .= "Informamos que o trabalho número ". $id_trabalho. ", com título \"";
    $mensagem .= $titulo . "\", ";
    $mensagem .= "submetido à 16ª Mostra de Pesquisa, Ensino e Extensão do IFRS - Câmpus Porto Alegre, ";
    $mensagem .= $msg;

    $mensagem .= "Esta mensagem foi gerada automaticamente pelo sistema de Inscrição.\n";
    $mensagem .= "http://mostra.poa.ifrs.edu.br/2015/sistema\n\n";
    ...
    $to = substr($to, 0, -2);
    $envia = mail("$to", "$assunto", "$mensagem", "from:Mostra IFRS Poa<mostra@poa.ifrs.edu.br>");
    //$envia = true;
    if ($envia == true) {
...
//$mensagem = "Prezado(a) Avaliador,\n
//O sistema de inscrição da Mostra estava com um problema e não registrou a confirmação de sessão. 
//Caso você tenha efetuado a confirmação, solicitamos que entre no sistema e efetue novamente.
//Pedimos desculpa pelo transtorno.\n";

//$mensagem .= "-----------------------------------\n".
 //             "Mensagem anteriormente enviada:\n".
  //            "-----------------------------------\n";

    $mensagem = "Prezado(a) Avaliador(a):\n\n";
    $mensagem .= "A Comissão Organizadora da 15ª Mostra de Pesquisa, Ensino e Extensão do IFRS - Câmpus Porto Alegre\n"
        . "tem a satisfação de confirmar sua participação como avaliador(a) nesse evento,\n"
        . "nos dias e horários informados a seguir.\n";

$mensagem .= " É imprescindível que confirme sua participação até o dia 09/09/2014, seguindo as instruções abaixo:\n".
   "- Acesse o sistema http://mostra.poa.ifrs.edu.br/2014/sistema\n".
   "- Digite seu login (CPF) e senha.\n".
   "- Clique em Área do Avaliador.\n".
   "- Confirme ou recuse suas sessões.\n\n".
        
   "Os resumos estarão disponíveis a partir do dia 10/09/2014 em http://mostra.poa.ifrs.edu.br/2014/site/html/trabalhos_aceitos.php.\n\n";

   //"Favor não responder este email. Se precisar entrar em contato, acesse o link Contato no site do evento.\n\n";
    ...
    while ($row_sessao = mysql_fetch_array($result_sessao)) {
      ...
      $assunto = "Mostra IFRS - Poa: Confirmação de Sessão (".$nome_sessao.") - ".$nome.".";
    ...
    $mensagem .= "Comissão Organizadora 15ª Mostra de Pesquisa,\n";
    $mensagem .= "Ensino e Extensão do IFRS - Câmpus Porto Alegre\n";
...
##############################################################################################################################
  ...
  while ($row = mysql_fetch_array($result_select)) {
    ...
    $mensagem = "15ª MOSTRA DE PESQUISA, ENSINO E EXTENSÃO DO IFRS – 
...
A Comissão Organizadora agradece sua disponibilidade e interesse em colaborar como avaliador(a) em nossa 15ª  Mostra de Pesquisa, Ensino e Extensão do IFRS, Porto Alegre.\n
...
Comissão Organizadora\n
15ª Mostra de Pesquisa, Ensino e Extensão\n
IFRS - Câmpus Porto Alegre";
...
##############################################################################################################################
else if (isset($_POST["option"]) && ($_POST["option"] == "enviar_email_autores_1")) {

  $status = STATUS_TRAB_ACEITO;

  $assunto = "Mostra IFRS POA 2014 - Retificação Regulamento.";

  $sql_seleciona_trabalhos = "SELECT * FROM trabalho t WHERE t.status = 4 AND t.fk_modalidade = 1 ";

  $executa_selecao_trab = runSQL($sql_seleciona_trabalhos);

$mensagem = "Prezado(a) autor(a),\n
inscrito na Modalidade Apresentação Oral:\n
Informamos a Retificação no Regulamento, no item 5.3, parágrafo 5º:\n\n

Onde se lia \"Fica sob a responsabilidade do apresentador a adequação de sua apresentação ao software Power Point 2003 e equipamentos disponíveis, bem como o armazenamento do arquivo em pen drive ou CD. \"\n\n

Agora lê-se: \"Fica sob a responsabilidade do apresentador a adequação de sua apresentação ao formato PDF e equipamentos disponíveis, bem como o armazenamento do arquivo em pen drive ou CD.\"\n\n";

  while ($row = mysql_fetch_array($executa_selecao_trab)) {
    $mensagem .="\nAtenciosamente,\n";
    $mensagem .="Comissão Organizadora da 15ª Mostra de Pesquisa, Ensino e Extensão do IFRS Câmpus Porto Alegre.\n";
    $mensagem .= "http://mostra.poa.ifrs.edu.br";
...
##############################################################################################################################
  ...
  $assunto = "Mostra IFRS POA 2014 - Pesquisa de Satisfação";
  ...
  while ($row = mysql_fetch_array($executa_selecao_trab)) {
    $mensagem = "Prezado(a) participante da 15ª Mostra de Pesquisa, Ensino e Extensão do IFRS Câmpus Porto Alegre,\n";
    $mensagem .= "A Comissão Organizadora está realizando uma pesquisa de satisfação referente ao evento. Por meio da sua opinião, poderemos aperfeiçoar o processo de realização do evento, na sua próxima edição. Acesse o link abaixo para responder a pesquisa. \n\n
	 
	 http://mostra.poa.ifrs.edu.br/2014/site/html/pesquisa.php \n
